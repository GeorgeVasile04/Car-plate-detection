...
def draw_yolo_bbox(image, bbox):
    """
    Draw a bounding box on an image from YOLO format coordinates.
    ...
    # Convert from YOLO format [x_center, y_center, width, height] to corner coordinates
    x_center = bbox[0] * w
    y_center = bbox[1] * h
    box_w = bbox[2] * w 
    box_h = bbox[3] * h
    ...
    x = int(x_center - box_w / 2)
    y = int(y_center - box_h / 2)
    box_w = int(box_w)
    box_h = int(box_h)
    
    # Draw the bounding box
    cv2.rectangle(image, (x, y), (x + box_w, y + box_h), (0, 1, 0), 2)
    
    # Draw center point
    cv2.circle(image, (int(x_center), int(y_center)), 4, (1, 0, 0), -1)
    ...
    for cx, cy in corners:
        cv2.circle(image, (cx, cy), corner_size, corner_color, -1)
